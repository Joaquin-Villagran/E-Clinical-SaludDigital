    ...
    <article className="rounded-[2rem] border border-[var(--border)] bg-[var(--background)] p-6">
      ...
      <div className="mt-4 grid gap-3 sm:grid-cols-2">
        ...
        <div className="rounded-[1.5rem] border border-[var(--border)] bg-[var(--card)] p-4 text-sm text-[var(--foreground)]/85">
          ...
          <p className="mt-1 text-xs text-[var(--muted)]">Modalidad solicitada: {turno.tipo_consulta === "presencial" ? "Consulta presencial" : "Videoconsulta"}</p>
        </div>
      </div>
      ...
      <div className="mt-5 flex flex-wrap gap-3">
        ...
        {(currentStatus === "confirmado" || currentStatus === "en_espera" || currentStatus === "en_consulta") && turno.tipo_consulta === "videoconsulta" ? <Link href={`/teleconsulta/${turno.id}`} className="inline-flex items-center gap-2 rounded-full bg-[var(--primary)] px-4 py-2 text-sm font-semibold text-white"><Video className="h-4 w-4" />Iniciar teleconsulta</Link> : null}
      ...
      {currentStatus === "confirmado" ? <div className="mt-4 grid gap-3 rounded-lg border border-[var(--border)] bg-[var(--card)] p-4 sm:grid-cols-[1fr_auto] sm:items-end"><label className="text-sm">Modalidad<select value={consultationType} onChange={(event) => setConsultationType(event.target.value)} className="mt-1 w-full rounded-md border border-[var(--border)] bg-[var(--background)] px-3 py-2"><option value="">Seleccionar</option><option value="presencial">Consulta presencial</option><option value="videoconsulta">Videoconsulta (Jitsi)</option></select></label><button type="button" onClick={() => handleAction("configurar_consulta")} disabled={loading || !consultationType} className="rounded-full bg-[var(--primary)] px-4 py-2 text-sm font-semibold text-white disabled:opacity-60">Guardar</button></div> : null}
