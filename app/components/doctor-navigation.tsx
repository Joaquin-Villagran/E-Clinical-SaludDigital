...
export default function DoctorNavigation() {
  ...
  const moreIsCurrent = pathname.startsWith("/panel/historial") || pathname.startsWith("/panel/mi-perfil");
  ...
  return <>
    <nav aria-label="Navegación del panel médico" className="hidden items-center gap-1 md:flex">
      ...
      <div className="relative">
        ...
        {isMoreOpen ? <div role="menu" className="absolute right-0 z-30 mt-2 w-52 rounded-lg border border-[var(--border)] bg-[var(--card)] p-2 shadow-[0_16px_40px_rgba(14,75,78,0.16)]">
          ...
          <Link role="menuitem" href="/panel/metricas" onClick={() => setIsMoreOpen(false)} className="flex items-center gap-3 rounded-md px-3 py-2 text-sm hover:bg-[var(--accent)]/10"><BarChart3 className="h-4 w-4 text-[var(--accent)]" />CRM y métricas</Link>
    ...
    {isMoreOpen ? <div className="fixed inset-x-4 bottom-20 z-40 rounded-lg border border-[var(--border)] bg-[var(--card)] p-2 shadow-[0_16px_40px_rgba(14,75,78,0.2)] md:hidden">
      ...
      <Link href="/panel/metricas" onClick={() => setIsMoreOpen(false)} className="flex items-center gap-3 rounded-md px-3 py-3 text-sm"><BarChart3 className="h-4 w-4 text-[var(--accent)]" />CRM y métricas</Link>
