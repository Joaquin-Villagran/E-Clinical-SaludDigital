...
export default function TeleconsultaPatientCard({ turno }: Props) {
  ...
  async function saveConfiguration() {
    ...
    try {
      const response = await fetch(`/api/turnos/${turno.id}`, {
        ...
        body: JSON.stringify({ action: "configurar_consulta", tipo_consulta: consultationType, meet_link: meetLink }),
      });
      ...
      setConfigMessage(error instanceof Error ? error.message : "No se pudo guardar la configuración.");
    } finally {
  ...
  return (
    <article className="grid gap-4 rounded-lg border border-[var(--border)] bg-[var(--card)] p-5 sm:grid-cols-[1fr_auto] sm:items-center">
      <div>
        <p className="text-xs font-semibold uppercase tracking-[.18em] text-[var(--accent)]">
          {isVideo ? "Videoconsulta confirmada" : "Configuración pendiente"}
        </p>
        <h2 className="mt-1 text-lg font-semibold text-[var(--primary)]">{turno.nombre}</h2>
        <p className="mt-1 text-sm text-[var(--muted)]">
          {turno.fecha_preferida} · {turno.hora_preferida} · {turno.duracion_minutos ?? 30} min
        </p>
        <p className="mt-2 text-sm">Motivo: {turno.motivo}</p>
      </div>
      ...
      <div className="flex flex-wrap gap-2 sm:justify-end">
        ...
        {!isVideo ? (
          <button
            type="button"
            onClick={() => setShowInformation(true)}
            className="inline-flex items-center justify-center rounded-full bg-[var(--primary)] px-4 py-2 text-sm font-semibold text-white"
          >
            Configurar
          </button>
        ) : turno.readyForRoom ? (
      ...
      {showInformation ? (
        <div
          role="dialog"
          aria-modal="true"
          aria-labelledby={`turno-${turno.id}-title`}
          className="fixed inset-0 z-50 flex items-end bg-black/30 p-4 sm:items-center sm:justify-center"
        >
          <section className="max-h-[92vh] w-full max-w-4xl overflow-y-auto rounded-lg bg-[var(--card)] p-6 shadow-[0_20px_60px_rgba(14,75,78,.25)]">
            <div className="flex items-start justify-between gap-4">
              <div>
                <p className="text-xs font-semibold uppercase tracking-[.18em] text-[var(--accent)]">Paciente confirmado</p>
                <h2 id={`turno-${turno.id}-title`} className="mt-1 text-2xl font-semibold text-[var(--primary)]">
                  {turno.nombre}
                </h2>
              </div>
              <button
                ...
                className="inline-flex h-9 w-9 items-center justify-center rounded-full text-[var(--foreground)] hover:bg-[var(--accent)]/10"
                title="Cerrar"
              >
                <X className="h-5 w-5" />
            ...
            <dl className="mt-6 grid gap-4 text-sm sm:grid-cols-2">
              <div>
                <dt className="text-[var(--muted)]">Fecha y hora del turno</dt>
                <dd className="mt-1 font-medium">
                  {turno.fecha_preferida} · {turno.hora_preferida}
                </dd>
              </div>
              <div>
                <dt className="text-[var(--muted)]">Motivo</dt>
                <dd className="mt-1 font-medium">{turno.motivo}</dd>
              </div>
            </dl>

            <div className="mt-6 border-t border-[var(--border)] pt-5">
              <p className="text-sm font-semibold text-[var(--primary)]">Configuración de la consulta</p>
              <div className="mt-3 grid gap-3">
                ...
                {configMessage ? <p className="text-sm text-[var(--primary)]">{configMessage}</p> : null}
              </div>
            </div>
            ...
            <div className="mt-6 border-t border-[var(--border)] pt-5">
              <FichaClinicaPanel
                ...
                prefill={{ nombreCompleto: turno.nombre, telefono: turno.telefono, email: turno.email, obraSocial: turno.obra_social }}
              />
            </div>
          </section>
        </div>
