...
export default async function PatientWaitingRoomPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  return <main className="min-h-screen bg-[var(--background)] text-[var(--foreground)]"><SiteHeader /><section className="container mx-auto max-w-3xl px-6 py-12"><WaitingRoom sessionHref={`/teleconsulta/${id}`} scheduledStart={scheduledStart} enabled={session.state === "active"} /></section></main>;
}